...
## Dependable refresh regressions — September 19, 2026

Added deterministic PRD-4 reliability coverage (`tests/reliability.test.ts`, 13 tests). Expired consent refreshes before fetch and publishes only with persisted rotated credentials; a failed refresh never fetches and keeps the stored snapshot. Login rejections retry exactly once through the persisted refresh token and never loop. Held leases skip sync untouched. Partial reads — undeclared account references, foreign-source rows, duplicate canonical keys, incomplete coverage and malformed timestamps — can never replace the stored complete snapshot. Unknown-custody aggregator rows are excluded only when a same-currency direct observation exists; merged duplicates throw. D1-verified: transient failures persist ERROR while expired consent persists NEEDS_LOGIN, both release the lease, retain the last snapshot and preserve the last success time; reconnect is generation-fenced per user and cannot overwrite another owner's connection. Dashboard and MCP responses verified to report the same snapshot, per-provider freshness, connection state and coverage limitations. All 52 tests, TypeScript and the Workers build pass. Owner-side live refresh-after-expiry and provider-receipt reconciliation remain open.
